 ...
 */
    ...
    */
      ...
      if (options.asSticker) mtype = MessageType.sticker
        ...
        if (/webp/.test(type.mime) && file.length <= 1 << 20) mtype = MessageType.sticker
          ...
          try { return await this.sendVideo(jid, file, caption, quoted, { ...opt, ...options }) }
            ...
            console.error('Error send video using sendVdeo, retrying using sendMessage... ', e)
    ...
    reply(jid, text, quoted, options) {
      return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, { contextInfo: { forwardingScore: 999, isForwarded: true, mentionedJid: this.parseMention(text), externalAdReply:{title: "ITSUKI BOTZ",body:"CREATED WITH ❤️ BY ITSUKI",previewType:"PHOTO",thumbnail:fs.readFileSync('../stik/thumb.jpeg'),sourceUrl:"https://wa.me/60199782326"}}, quoted, ...options })
    }
